...
namespace LinguaKoglin
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //verificando se a letra é tipo foo ou tipo bar

            bool isFoo(char letter)
            {
              
              if (letter.Equals('s') || letter.Equals('l') || letter.Equals('f') || letter.Equals('w') || letter.Equals('k')){
                  return true;
              }
              else
              {
            ...
            //contador de preposições
            int CountPreposition(string text)
            {
                string[] wordsTexto = text.Split(' ');
                int count = 0;

                foreach (var word in wordsTexto)
                {
                    if(word.Length == 3 && isFoo(word[2])== false && !word[2].Equals('d'))
                    count++;                                      
                }
                return count;
            }

            string CountVerb(string text)
            {
                string[] wordsTexto = text.Split(' ');
                int countVerb = 0;
                int countFirstPerson = 0;

                foreach(var word in wordsTexto)
                {

                    if(word.Length >= 8 && isFoo(word[word.Length - 1]) == true)
                    {
                        countVerb++;
                        if(isFoo(word[0]) == false){
                            countFirstPerson++;
                        } 
                    }
                }

                return "Contagem de Verbos: " + countVerb + "\nContagem de verbos em primeira pessoa: " + countFirstPerson;

            }


            List<string> VocabularyList(string text)
            {

                string order = "kbwrqdnfxjmlvhtcgzps";
                char[] alphabet = order.ToCharArray();
                List<string> list = new List<string>();

                string[] wordsTexto = textA.Split(' ');
            ...
            void WordToNumber(string word)
            {

                for (int i = 0; i < word.Length;)
                {
                   Console.WriteLine(OrderNumber(word[i]));
                }


            }

            int OrderNumber(char letter)
            {
                string order = "kbwrqdnfxjmlvhtcgzps";
                char[] alphabet = order.ToCharArray();
                int i = 0;
            ...
            //Console.WriteLine(CountVerb(textB));
            //Console.WriteLine(CountPreposition(textB));
            //Console.WriteLine(isFoo('s'));
